<script lang="ts">
	...
	// Function to extract model parameter size and quantization level
	function extractModelInfo(model) {
		let paramSize = '';
		let quantLevel = '';

		// First try to get values from ollama details (most reliable)
		if (model?.ollama?.details?.parameter_size) {
			paramSize = model.ollama.details.parameter_size;
		}

		if (model?.ollama?.details?.quantization_level) {
			quantLevel = model.ollama.details.quantization_level;
		}

		// If parameter size isn't available, try to extract from name
		if (!paramSize && model?.name) {
			const paramMatch = model.name.match(/(\d+(\.\d+)?[BbMmTt])/);
			if (paramMatch) {
				paramSize = paramMatch[1];
			}
		}

		// If quantization isn't available, try to extract from name
		if (!quantLevel && model?.name) {
			// Look for common quantization formats after the parameter size
			const quantMatch = model.name.match(/[-_](F16|F32|Q\d+(_\d+)?(_[KM])?)/i);
			if (quantMatch) {
				quantLevel = quantMatch[1];
			}
		}

		return {
			paramSize: paramSize || 'Unknown',
			quantLevel: quantLevel || 'Unknown'
		};
	}

	// Organize models by parameter size and quantization
	$: organizedModels = (() => {
		const byParam = {};
		...
		return Object.entries(byParam)
			...
			.sort((a, b) => {
				...
				const numA = parseFloat(a.paramSize.match(/(\d+(\.\d+)?)/)?.[1] || '0');
				const numB = parseFloat(b.paramSize.match(/(\d+(\.\d+)?)/)?.[1] || '0');
	...
	>
		<slot>
			...
			<div class="px-3 max-h-64 overflow-y-auto scrollbar-hidden group relative">
				...
				{#if organizedModels.length > 0}
					{#each organizedModels as paramGroup}
						<div class="mt-2 mb-1">
							<!-- Parameter size header -->
							<div class="px-3 py-1 text-xs font-medium text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-700">
